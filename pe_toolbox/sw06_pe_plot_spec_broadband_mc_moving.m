...
if ~exist(spec_filename,'file')
    ...
    duration_sec=0:5:480;
    freq=[270:2:330];
    psi2=zeros(length(phone_depth),length(duration_sec), length(freq));
    for ix_sec = 1:length(duration_sec)
        ...
        for ix_freq = 1:length(freq)
            ...
            psi2(:,ix_sec,ix_freq) = psifinal(phone_index,ixy);
...
for ix=1:length(phone_depth)
    ...
    pcolor(duration_sec/60,freq,20*log10(abs(squeeze(psi2(ix,:,:)))'))
    axcb = colorbar;
    shading interp
    caxis([-60 -42])
