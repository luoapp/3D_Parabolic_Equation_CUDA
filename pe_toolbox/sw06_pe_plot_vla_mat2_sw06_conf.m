...
for sourcename1 = {'nrl300'}
    ...
    for ix_t = 3%: size(trans.time,1)
        ...
        for ix_y = floor(length(pe.y)/2) %+[-1500:10:1500]
            ...
            %pause
            lj_saveSameSize(sprintf('%s%s%d/SW06_PE_%s_%s%d_VLA_y%d_%s.png',figdir,transid,ix_t,sourcename,transid,ix_t,n,envtype))
