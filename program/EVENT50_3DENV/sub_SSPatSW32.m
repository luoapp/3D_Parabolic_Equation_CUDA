function [SSP, SENSOR_DEPTH, WATERDEPTH, TEMPR, SALT] = sub_SSPatSW32(DATA,TIME,isplotcontour)
% Extract SSP profiles at the SW32
%    [SSP, SENSOR_DEPTH, WATERDEPTH, TEMPR, SALT] = sub_SSPatSW32(DATA_STRUCTURE,TIME,[isplotcontour])
% Input:
...
SSP(:,find(IDT>0&IDT<=117989)) = DATA.SNDSPD(:,IDT(find(IDT>0&IDT<=117989)));
SENSOR_DEPTH(:,find(IDT>0&IDT<=117989)) = DATA.SENSOR_DEPTH(:,IDT(find(IDT>0&IDT<=117989)));
WATERDEPTH(:,find(IDT>0&IDT<=117989)) = DATA.WATERDEPTH(:,IDT(find(IDT>0&IDT<=117989)));
TEMPR(:,find(IDT>0&IDT<=117989)) = DATA.T(:,IDT(find(IDT>0&IDT<=117989)));
SALT(:,find(IDT>0&IDT<=117989)) = DATA.SALT(:,IDT(find(IDT>0&IDT<=117989)));
